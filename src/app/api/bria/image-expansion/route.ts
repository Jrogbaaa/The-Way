...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const direction = requestFormData.get('direction') as string | null;
    const prompt = requestFormData.get('prompt') as string | null;
    ...
    console.log('Received request for /api/bria/image-expansion');
    console.log(`Processing image: ${imageFile.name}, Direction: ${direction}, Prompt: ${prompt || 'N/A'}`);

    const endpoint = `${BRIA_BASE_URL}/image_expansion`;
    ...
    console.log('Successfully received response from Bria AI /image_expansion');
    ...
    console.error('Error in /api/bria/image-expansion:', error);
    const errorMessage = error.message || 'Failed to expand image';
