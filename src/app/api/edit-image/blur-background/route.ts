...
export async function POST(req: NextRequest) {
  try {
    console.log('POST /api/edit-image/blur-background: Starting request processing');
    ...
    console.log('Using BRIA AI background/blur API...');
    
    // Initialize the BRIA client with the API key
    const briaClient = new BriaClient(processEnv.BRIA_API_KEY);
    
    // Use the blurBackground method
    const result = await briaClient.blurBackground(imageBuffer);
    ...
    console.error('Error in blur-background API route:', error);
    ...
    if (axiosError.response) {
      ...
      return NextResponse.json(
        { error: 'Failed to blur background', details: errorData },
    ...
    return NextResponse.json(
      { error: 'Failed to blur background', message: axiosError.message || 'Unknown error' },
      { status: 500 }
    );
  }
} 