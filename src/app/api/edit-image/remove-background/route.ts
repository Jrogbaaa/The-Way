...
export async function POST(req: NextRequest) {
  try {
    console.log('POST /api/edit-image/remove-background: Starting request processing');
    ...
    console.log('Using BRIA AI background/remove API...');
    
    // Initialize the BRIA client with the API key
    const briaClient = new BriaClient(processEnv.BRIA_API_KEY);
    
    // Use the removeBackground method
    const result = await briaClient.removeBackground(imageBuffer);
    ...
    if (axiosError.response) {
      ...
      return NextResponse.json(
        { error: 'Failed to remove background', details: errorData },
    ...
    return NextResponse.json(
      { error: 'Failed to remove background', message: axiosError.message || 'Unknown error' },
      { status: 500 }
    );
  }
} 