...
const processEnv = {
  BRIA_AI_API_KEY: process.env.BRIA_AI_API_KEY || ''
};

export async function POST(req: NextRequest) {
  try {
    console.log('POST /api/edit-image/inpaint: Starting request processing');
    ...
    if (!imageFile || !maskFile || !prompt) {
      console.error('Image, mask or prompt missing from request');
    ...
    const briaClient = new BriaClient(processEnv.BRIA_AI_API_KEY);
    console.log('Calling BRIA editImage API');
    
    // Call the BRIA editImage method
    const result = await briaClient.editImage(imageBuffer, maskBuffer, prompt);
    ...
    if (axiosError.response) {
      ...
      return NextResponse.json(
        { error: 'Failed to inpaint image', details: errorData },
    ...
    return NextResponse.json(
      { error: 'Failed to inpaint image', message: axiosError.message || 'Unknown error' },
      { status: 500 }
    );
  }
} 