...
5. **Outpaint**: Extends the image beyond its original boundaries in any direction (using BRIA AI)
...
4. **Implementation details**:
   - Utilizes our custom BriaClient SDK
...
2. **Implementation details**:
   - Uses BRIA AI's image edit endpoint
...
2. **BRIA AI** for advanced image editing features:
   - Custom BriaClient SDK in `src/lib/bria-sdk.ts` handles all BRIA AI interactions
   - Supports multiple editing functionalities:
     - **Outpainting**: Extends images in multiple directions using the image expansion endpoint
...
Our custom BriaClient provides:

1. **Image Preparation**:
   - Automatically resizes images that exceed size limits
...
2. **Outpainting Implementation**:
   ...
   - Uses the image expansion endpoint to generate new content in the masked areas
...
3. **Search & Replace and Recolor**:
   ...
   - Uses the gen_fill endpoint with carefully crafted parameters
   - Includes appropriate handling of multipart/form-data for file uploads
...
BRIA_AI_API_KEY=your_bria_api_key_here
BRIA_AI_BASE_URL=https://api.bria.ai
```
